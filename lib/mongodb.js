...
function getHostName(url) {
  ...
  if (lastIndex) host = host.substring(0, lastIndex);
  return host;
}
...
function MongoDB(settings, dataSource) {
  ...
  this.settings.useNewUrlParser = true;
  this.settings.useUnifiedTopology = true;
...
MongoDB.prototype.connect = function(callback) {
  const self = this;
  const hostname = getHostName(self.settings.url);
  ...
  if (self.db) {
  ...
  } else if (dBConnection && dBConnection[hostname]) { // afterForkChanges
    try {
      ...
      const url = urlParser(self.settings.url, validOptions);
      self.db = dBConnection[hostname].db(
    ...
    //   client,
    // ) {
    new mongodb.MongoClient(self.settings.url, validOptions).connect().then(
      client => {
        if (self.debug) {
          debug('MongoDB connection is established: ' + self.settings.url);
        }
        self.client = client;
        dBConnection[hostname] = self.client;
        ...
        try {
          ...
          if (callback) callback(undefined, self.db);
        } catch (e) {
          onError(e);
        }
      },
    ...
    self.notifyObserversAround(
      ...
      function(context, done) {
        ...
        if (command === 'find') {
          ...
          const localPromise = collection[command].apply(collection, args);
          localPromise.then(result => callback(undefined, result), error => callback(error));
          return localPromise;
        }
      },
      callback,
...
MongoDB.prototype.getDatabaseColumnName = function(model, propName) {
  ...
  // mongoDB connector doesn't support custom id property field name
  if (prop.id) {
    // throws if a custom field name for id is set
    const customFieldName = (prop.mongodb && (
      ...
      prop.column;
    if (customFieldName) {
      throw new Error(`custom id field name '${customFieldName}' is not allowed` +
      ` in model ${model.model.definition.name}`);
    }
    return propName;
  } else {
    // Try mongo overrides
  ...
  if (self.db) {
    ...
    async.eachSeries(
      ...
      function(modelName, modelCallback) {
        ...
        if (!indexList || !indexList.length) return modelCallback();
        const collection = self.collection(modelName);
        if (!collection) return modelCallback();
        collection.indexes()
        .catch(err => {
          // Ignore error if the collection doesn't exists
          if (err.codeName !== 'NamespaceNotFound') {
            throw err; // Rethrow if it's a different error
          }
        }).then(existingIndexes => {
          if (existingIndexes && existingIndexes.length) indexList = indexList.filter(index => !existingIndexes.find(existingIndex => index.options && existingIndex.name === index.options.name));
            ...
            async.eachSeries(
              ...
              function (index, indexCallback) {
                ...
                collection.createIndex(
                  ...
                  index.options,
                  indexCallback,
                ).then(res => indexCallback(undefined, res))
                  .catch(err => indexCallback(err));
              },
              modelCallback,
            );
          })
...
function isObjectIDProperty(modelCtor, propDef, value, options) {
  ...
  } else if (value instanceof mongodb.ObjectId || (value && value.constructor.name === 'ObjectID')) {
    return true;
  } else {
    return false;
  }
