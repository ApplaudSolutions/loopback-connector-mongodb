...
MongoDB.prototype.connect = function(callback) {
  ...
  if (self.db) {
    ...
    self.db = dBConnection[hostname].db(self.settings.name);
    process.nextTick(function() {
      callback && callback(null, self.db);
    });
  } else {
...
MongoDB.prototype.disconnect = function(cb) {
  ...
  if(dBConnection && Object.keys(dBConnection).length > 0) {
    Object.keys(dBConnection).forEach(function(k) {
      if(dBConnection[k]) {
